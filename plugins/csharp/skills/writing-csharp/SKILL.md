...
- Don't test what you don't own. Never write a test proving a framework or library does its job — that EF Core saves, that `System.Text.Json` round-trips — that's their contract, not your logic. Don't mock what you don't own either: wrap the foreign type in a port and substitute a fake for the port.
