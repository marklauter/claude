...
- Make invalid states unrepresentable. Invariants are established at construction; once a value exists it is trusted and never re-checked. Validation lives in a `Result`-returning `static Result<T> Parse(...)` at untrusted boundaries — it owns every check; a trusted `static T Create(TValue)` takes validity as a precondition and never validates (violating it is a caller bug). `required`/`init` members, never `set`. Discriminated unions (abstract base record + sealed per-state records) for state machines; pattern-match to handle. Parse, don't validate.
- Wrap primitives that carry meaning. A `readonly record struct` value type via `IValue` — `FilePath` over `string`, `Percent` over `decimal` — so the constraint is carried by a distinct nominal type and a bare `string` or `decimal` is no longer assignment-compatible with a checked value.
