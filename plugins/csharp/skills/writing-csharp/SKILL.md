...
- Make invalid states unrepresentable. Construction is where invariants land; once a value exists it is trusted and never re-checked. Private constructor + `public static Result<Range> Create(...)` that validates. `internal sealed` records and classes by default, `public` only for exported contract, `public interface` over `internal sealed` implementation. `required`/`init` members, never `set`. `readonly record struct` wrappers via `IValue<TSelf, TValue>` for primitives that carry meaning — `FilePath` over `string`, `Percent` over `decimal`. Discriminated unions (abstract base record + sealed per-state records) for state machines; pattern-match to handle. Parse, don't validate.
...
- Compose functions; push effects to the edge. Expressions over statements; `map`/`bind`/`Match` and LINQ pipelines over `for` loops and hand-rolled branching; thread `Result<T>`/`Either<L, R>` through `bind` instead of unwrapping and re-checking. I/O — console, file, network, database, clock — lives at the boundary through ports and adapters; the core takes data in and returns data out. Static pure transforms (tokenize, normalize, parse, format): no state, no I/O, no logging. Inject `TimeProvider` over `DateTime.UtcNow`. A test that needs mocks or async setup means the function isn't pure.
