...
Write a functional core and model it as a domain: pure and immutable, effects pushed to the edge, expressed with sum types, Evans' aggregates, and bounded contexts. Pure functions are referentially transparent, so a call reduces to its value and reasoning stays local. Immutability removes the shared mutable state that makes concurrent code unsafe.
...
- Make invalid states unrepresentable. `@dataclass(frozen=True, slots=True)` for records: `frozen` blocks reassignment and `slots` rejects attributes nobody declared. Add `kw_only=True` once a record has three or more fields, or any two of the same type, so a transposed argument can't type-check. Enforce the invariant in `__post_init__` and raise there, because Python has no private constructor to hide an unchecked path behind.
- Parse, don't validate. A `parse` classmethod at each untrusted boundary is the fallible lift `str → Self | Invalid`, where `Invalid` is a variant carrying the reason, and it is the only place external input becomes a domain value. It catches what `__post_init__` raises and returns that as a value. Type the incoming payload as a `TypedDict` at the adapter and convert it inside `parse`; no domain function takes a `dict`. Nothing downstream re-checks: a value that exists is valid.
...
- Compose with expressions. Comprehensions for the common transform, generator expressions for lazy streams, `itertools` (`chain`, `groupby`, `takewhile`, `accumulate`) for pipelines, `functools` (`partial`, `cache`) and `operator` (`itemgetter`, `add`) for the function forms. Fold with `sum`, `math.prod`, or a loop over an accumulator rather than `functools.reduce`, which Python discourages. Return early on a failure variant instead of nesting the happy path one level deeper at each check. Replace deep recursion with a loop; Python eliminates no tail calls.
- Push effects to the edge (functional core, imperative shell). I/O — console, file, network, database, clock — lives at the boundary behind a `Protocol` port; the core takes data in and returns data out. A pure transform (tokenize, normalize, parse, format) has no state, no I/O, and no logging. Inject the clock and the connection; never call `datetime.now()` or reach for a module-level client inside a transform. Keep import time free of side effects and guard entry points with `if __name__ == "__main__":`. A function that needs a test double or async setup isn't pure.
- Keep the core synchronous; `async` is an edge concern. Coroutines belong in adapters and the entry point, so the core stays callable without an event loop and its tests need no async fixtures. Use `asyncio.TaskGroup` over a bare `gather`: it cancels siblings on failure and raises an `ExceptionGroup`, where `gather` surfaces one exception and leaves the rest unretrieved. Put a timeout on every await that crosses the network, and hand a blocking library to `asyncio.to_thread` rather than calling it from a coroutine.
...
- Model every operation as a total function; its codomain picks the channel. Modeled outcomes are values: success and every failure the operation's own logic produces (a parse rejecting input, a withdrawal exceeding balance, a lookup finding nothing). Carry them in `T | None` for plain absence, or in a union of sibling frozen dataclasses where the failure carries context: name the variants the domain acts on and return `T | NotFound | Conflict`. Don't import a `Result` monad for this — the union is stdlib, pyright checks it exhaustively, and it reads as Python where `.bind()` chains read as transliterated F#. The pure core is total: invalid states are unrepresentable, so it cannot produce an invariant violation and never raises. Results originate at the boundary, parsing representable-invalid input into a domain type.
...
Lint, types, and tests are the ground truth between sessions; fix what fires, don't silence it. Configure once in `pyproject.toml`: `ruff` for lint and format (it replaces black, flake8, isort, and pyupgrade), `pyright` in strict mode, `pytest` for tests, `uv` for dependencies and the lockfile. Declare `requires-python = ">=3.12"`, which the `type` statement, `@override`, and PEP 695 aliases all need. Put the coverage ratchet in `addopts` — `--cov=src --cov-branch --cov-fail-under=<N>` — so the project-wide run enforces it; the threshold is monotonic, set to the measured value and only ever raised. Encode architectural invariants (layer dependencies, persistence ignorance, no adapter import reaching into the core) as `import-linter` contracts asserted by a test, so they run inside the same pytest step that runs everything else.

`src/<package>/` for code, `tests/` mirroring it (`src/pkg/parser.py` ↔ `tests/test_parser.py`), `__init__.py` in every package directory. Inside the package, three layers with one dependency direction: `domain/` holds the pure types and transforms and imports nothing from its siblings, `adapters/` holds the I/O behind the ports `domain` declares, and `app/` is the composition root and entry points and may import both. `domain` importing from `adapters` or `app` is what the `import-linter` contract forbids.

Validate with the canonical script at `${CLAUDE_PLUGIN_ROOT}/skills/writing-python/scripts/build-gate.sh` — fail-fast format, lint, types, test: each step runs only when the prior is green, and one exit code is the gate. It verifies only; it never edits. By default it runs project-wide; it also takes a scoped pytest target, which skips coverage because a subset of the suite can't meet a project-wide floor. Run `build-gate.sh --help` for the invocations.
